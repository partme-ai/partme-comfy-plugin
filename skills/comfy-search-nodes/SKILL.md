...
description: "Search for ComfyUI nodes available on Comfy Cloud: $ARGUMENTS"
license: Apache-2.0
---
...
1. 用户明确要搜索并评估 Comfy 自定义节点及其依赖风险。
2. 已提供或可安全取得必要上下文，需要得到可验证的 `node-search-report`。
...
Step 4：生成或执行 `node-search-report`，每一步都绑定输入、预期输出与失败条件。
...
- [ ] 目标、环境与授权范围均已写明。
- [ ] `节点来源、版本、依赖、维护状态和替代方案可验证` 已由可复现证据验证。
- [ ] 敏感数据已脱敏，输出中没有完整凭据。
- [ ] 失败与超时路径已覆盖，未出现无限重试。
- [ ] 变更类任务具有备份或回滚说明。
- [ ] 最终结论区分事实、推断和未验证项。
...
2. **成功码不等于业务成功**：必须检查 `节点来源、版本、依赖、维护状态和替代方案可验证`，不能只看命令退出码或 HTTP 200。
