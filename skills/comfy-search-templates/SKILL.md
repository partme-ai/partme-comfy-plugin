...
description: "Search for ComfyUI workflow templates available on Comfy Cloud: $ARGUMENTS"
license: Apache-2.0
---
...
1. 用户明确要搜索并筛选可复用的 Comfy 工作流模板。
2. 已提供或可安全取得必要上下文，需要得到可验证的 `template-search-report`。
...
Step 4：生成或执行 `template-search-report`，每一步都绑定输入、预期输出与失败条件。
...
2. **成功码不等于业务成功**：必须检查 `模板来源、依赖、输入输出和适配成本完整`，不能只看命令退出码或 HTTP 200。
